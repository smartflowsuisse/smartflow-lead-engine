# Outreach Batch 001 — Manual Review

Purpose: prepare the first real manual outreach batch for SmartFlow Suisse.

Rules:
- Do not send automatically.
- Open each website manually.
- Check if the company is real and relevant.
- Find one concrete quick win before writing.
- Send only after manual review.
- After sending, mark the lead as Contacted.

## Candidates

### 1. AG Construction SA

- City: Genève
- Industry: Construction
- Email: info@ag-construction.ch
- Website: https://www.ag-construction.ch
- Lead score: 73
- CRM status: Analyzed
- Outreach status: New

Manual review:
- Website opens: YES
- Company relevant: YES — construction / real estate projects in Suisse romande
- Quick win found: YES — mobile performance is weak; Lighthouse mobile shows Performance 53, while  strong at 92
- Suggested offer: small website audit + mobile performance/contact-path quick win
- Email approved: DRAFT READY — not sent
- Mini-audit prepared: YES — docs/outreach/audits/AG_CONSTRUCTION_MINI_AUDIT_FR.md
- Email draft: YES — docs/outreach/emails/AG_CONSTRUCTION_EMAIL_DRAFT_FR.md
- Sent manually: NO

---

### 2. Swissroc Construction

- City: Genève
- Industry: Construction
- Email: contact@swissroc.com
- Website: https://swissroc.ch/fr/services/construction/
- Lead score: 72
- CRM status: Analyzed
- Outreach status: New

Manual review:
- Website opens: TODO
- Company relevant: TODO
- Quick win found: TODO
- Suggested offer: TODO
- Email approved: TODO
- Sent manually: TODO

---

### 3. Implenia Suisse SA

- City: Crissier
- Industry: Construction
- Email: communication@implenia.com
- Website: https://implenia.com/en/
- Lead score: 68
- CRM status: Analyzed
- Outreach status: New

Manual review:
- Website opens: TODO
- Company relevant: TODO
- Quick win found: TODO
- Suggested offer: TODO
- Email approved: TODO
- Sent manually: TODO

---

### 4. Kurt Keller AG

- City: Zürich
- Industry: Bauunternehmen
- Email: info@kkag.ch
- Website: https://kkag.ch
- Lead score: 45
- CRM status: New
- Outreach status: New

Manual review:
- Website opens: TODO
- Company relevant: TODO
- Quick win found: TODO
- Suggested offer: TODO
- Email approved: TODO
- Sent manually: TODO

---

