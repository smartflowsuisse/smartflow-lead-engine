...
describe("template pack registry", () => {
  it("registers construction and retail template packs", () => {
  ...
  it("keeps template pack options available for future UI selectors", () => {
    assert.equal(templatePackOptions.length, 2);

    assert.deepEqual(
      templatePackOptions.map((option) => option.id).sort(),
      ["construction", "retail"],
    );
  });
  ...
  it("keeps all registered packs multilingual", () => {
    const packIds: TemplatePackId[] = ["construction", "retail"];
